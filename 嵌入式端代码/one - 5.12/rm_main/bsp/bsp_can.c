...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	...
	else if(hcan == &hcan2 )
	{
		...
		switch (Rx2Message.StdId)
		{
			...
			case CAN_TRIGGER_MOTOR2_ID:
			{
				motor_trigger2.msg_cnt++ <= 50 ? get_moto_offset(&motor_trigger2, &hcan2,CAN2_Rx_data) : encoder_data_handler(&motor_trigger2,&hcan2,CAN2_Rx_data);
				break;
			}
		}
...
void can2_send(int16_t TX_ID,uint8_t power_limet,uint8_t ID,int16_t cooling_heat_limet,int16_t speed_limit,int16_t cooling_rate)
{
	uint8_t FreeTxNum = 0; 
  ...
  Tx2Message.DLC   = 0x08;
	...
	CAN2_Tx_data[0] = power_limet;
	CAN2_Tx_data[1] = ID;
	CAN2_Tx_data[2] = cooling_heat_limet>>8;
	CAN2_Tx_data[3] = cooling_heat_limet;
	CAN2_Tx_data[4] = speed_limit>>8;
	CAN2_Tx_data[5] = speed_limit;
	CAN2_Tx_data[6] = cooling_rate>>8;
	CAN2_Tx_data[7] = cooling_rate;
